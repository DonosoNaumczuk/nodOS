...
int semaphoreShow(int argumentQuantity, void **argumentVector) {
	...
	argument[2] = 0;
	argument[3] = 0;
	argument[4] = relatorSem;
	argument[5] = kungFurySem;
	argument[6] = hackerManSem;
	pid[0] = createProcess(&relator, 7, argument);
	argument[1] = kungFurySem;
	pid[1] = createProcess(&kungFury, 7, argument);
	argument[1] = hackerManSem;
	pid[2] = createProcess(&hackerMan, 7, argument);
	waitChild(pid[0]);
	waitChild(pid[1]);
	waitChild(pid[2]);
	return 0;
